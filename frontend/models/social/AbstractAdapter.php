...
abstract class AbstractAdapter implements AdapterInterface
{
     ...
     * @return mixed
     */
    protected function get($url, $field, $params, $parse = true)
    {
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url . $field . urldecode(http_build_query($params)));
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
        $result = curl_exec($curl);
        curl_close($curl);
    ...
    // protected function getInsta($url, $parse = true)
    // {
    //     $curl = curl_init();
    //     curl_setopt($curl, CURLOPT_URL, $url);
    //     curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
    ...
    //     return $result;
    // }
    protected function getInsta($url,$access_token_parameters, $parse = true)
    {
        $curl = curl_init($url);
